...
// NOTE: seed ไฟล์นี้จงใจ seed แค่ reference data ที่เป็นของเราเอง (categories + merchants/MCC)
// ข้อมูล promotions / card_base_benefit เป็นข้อมูลจริงจากธนาคาร ต้องมี source_url กำกับทุกแถว
// จึงไม่ seed จากไฟล์นี้ — ให้เข้าทาง import script หรือ Admin portal เท่านั้น
// (ของเดิมมี 20 promotions ที่แต่งขึ้น ลบออกแล้ว เพราะไม่มีแหล่งอ้างอิงและค่าไม่สมจริง)
async function main() {
  // ── Categories ──────────────────────────────────────────────
  ...
  console.log(`✅ Seeded: ${CATEGORIES.length} categories, ${MERCHANTS.length} merchants`)
}
